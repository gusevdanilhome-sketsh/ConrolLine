...
void BayesianClassifier::setNoiseStd(double sigma) { noiseStd_ = sigma; }

void BayesianClassifier::setParameterGrid() { buildClassModels(); }
...
void BayesianClassifier::buildClassModels() {
  ...
  // класс 0
  classModels_[0].featureVectors.push_back(
      gen.computeFeaturesForParams(0, 0.0, freqs));
  classModels_[0].weights.push_back(1.0);
  // класс 1
  ...
  for (double Ld : Ld_vals)
    for (double Rd : Rd_vals) {
      classModels_[1].featureVectors.push_back(
          gen.computeFeaturesForDefect(1, Ld, Rd, freqs));
      classModels_[1].weights.push_back(1.0 / total);
    }
  // класс 2
  std::vector<double> Cd_vals = {0.01e-12, 0.05e-12, 0.1e-12, 0.15e-12,
                                 0.2e-12};
  total = Cd_vals.size();
  for (double Cd : Cd_vals) {
    classModels_[2].featureVectors.push_back(
        gen.computeFeaturesForDefect(2, Cd, 0.0, freqs));
    classModels_[2].weights.push_back(1.0 / total);
  }
  // класс 3
  std::vector<double> Csh_vals = {0.02e-12, 0.05e-12, 0.1e-12, 0.2e-12,
                                  0.3e-12};
  total = Csh_vals.size();
  for (double Csh : Csh_vals) {
    classModels_[3].featureVectors.push_back(
        gen.computeFeaturesForDefect(3, Csh, 0.0, freqs));
    classModels_[3].weights.push_back(1.0 / total);
  }
  // класс 4
  std::vector<double> Csh2_vals = {0.01e-12, 0.05e-12, 0.1e-12, 0.15e-12,
                                   0.25e-12};
  total = Csh2_vals.size();
  for (double Csh : Csh2_vals) {
    classModels_[4].featureVectors.push_back(
        gen.computeFeaturesForDefect(4, Csh, 0.0, freqs));
    classModels_[4].weights.push_back(1.0 / total);
  }
  numFeatures_ = (int)classModels_[0].featureVectors[0].size();
}
...
double BayesianClassifier::computeLogLikelihood(const std::vector<double> &x,
  ...
  for (size_t k = 0; k < model.featureVectors.size(); ++k) {
    ...
    double logLik =
        -0.5 * sumSq * invVar -
        0.5 * numFeatures_ * std::log(2 * M_PI * noiseStd_ * noiseStd_);
...
std::vector<std::vector<double>> BayesianClassifier::predictProbabilities(
    const std::vector<std::vector<double>> &X) const {
  if (!trained_ || X.empty())
    return {};
  std::vector<std::vector<double>> probs(X.size(),
                                         std::vector<double>(numClasses_, 0.0));
  for (size_t i = 0; i < X.size(); ++i) {
    std::vector<double> logProbs(numClasses_);
    for (int c = 0; c < numClasses_; ++c)
      logProbs[c] =
          std::log(priors_[c]) + computeLogLikelihood(X[i], classModels_[c]);
